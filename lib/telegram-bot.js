...
import { getAIResponse, processOperatorMessage, addToHistory, clearHistory } from './openrouter.js'
    ...
    if (chatType === 'supergroup' || chatType === 'group') {
      if (chatId.toString() === RECEPTION_GROUP_ID?.toString() && message.message_thread_id) {
        ...
        try {
          ...
          const { action, text: aiText } = await processOperatorMessage(text, userId)
          const botName = await getBotName()
