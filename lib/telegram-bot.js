    ...
    if (chatType === 'supergroup' || chatType === 'group') {
      if (chatId.toString() === RECEPTION_GROUP_ID?.toString() && message.message_thread_id) {
        ...
        if (SCHEMA_TOPIC_ID && topicId === SCHEMA_TOPIC_ID) {
          ...
          try {
            const { schema, imageUrl, generatedPrompt } = await processSchemaMessage(text)

            // Отправляем картинку схемы
            try {
              await bot.telegram.sendPhoto(RECEPTION_GROUP_ID, imageUrl, {
                message_thread_id: topicId,
